...
const sources = sourcesJson as SourceInput[];
...
function classifySource(source: SourceInput) {
  const baseScore = {
    official: 88,
  ...
  return { confidence, status: "needs_review" };
}

function inferRiskTags(source: SourceInput) {
  const tags: string[] = [];
...
function buildDealCandidate(source: SourceInput) {
...
export async function ingestMonitoredSources() {
  if (!hasDatabase()) {
    return { sources: sources.length, candidates: sources.length, upserted: false };
  }
  ...
  for (const deal of candidates) {
    ...
    await pool.query("update sources set last_checked_at = now() where url = $1", [deal.source_url]);
  }

  return { sources: sources.length, candidates: candidates.length, upserted: true };
}
